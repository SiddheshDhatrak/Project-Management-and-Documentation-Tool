...
export const AppProvider = ({ children }) => {
  ...
  // Version history operations (backend-backed)
  const createVersion = async (pageId, content) => {
    try {
  ...
  const getVersions = async (pageId) => {
    try {
      return await versionsAPI.getByPage(pageId);
    } catch (e) {
  ...
  const restoreVersion = async (pageId, versionId) => {
    try {
      const versions = await versionsAPI.getByPage(pageId);
